...
public class BlockInteractionEvents {
	...
	private static void handleAncientAltarClick(Player player, net.minecraft.core.BlockPos pos, Level level) {
		...
		String heldKey = net.minecraft.core.registries.BuiltInRegistries.ITEM.getKey(player.getMainHandItem().getItem())
				.toString();
		if (!heldKey.equalsIgnoreCase(com.nightbeam.remnants.config.JaumlConfigLib.getStringValue("remnant/bosses",
				"ossukage_summon", "portal_activation_item")))
			return;

		// Verify pedestals are placed at +/-3 on X/Z
		if (level.getBlockState(pos.offset(3, 0, 0)).getBlock() != com.nightbeam.remnants.init.ModBlocks.ANCIENT_PEDESTAL
				.get()
				|| level.getBlockState(pos.offset(-3, 0, 0))
						.getBlock() != com.nightbeam.remnants.init.ModBlocks.ANCIENT_PEDESTAL.get()
		...
		net.minecraft.resources.ResourceLocation cfgOne = net.minecraft.resources.ResourceLocation
				.parse(com.nightbeam.remnants.config.JaumlConfigLib
			...
			if (e instanceof net.minecraft.server.level.ServerPlayer _sp) {
				...
				if (player.getInventory().contains(player.getMainHandItem())) {
					if (net.minecraft.core.registries.BuiltInRegistries.ITEM.getKey(player.getMainHandItem().getItem())
							.toString()
							.equalsIgnoreCase(com.nightbeam.remnants.config.JaumlConfigLib.getStringValue("remnant/bosses",
									"ossukage_summon", "portal_activation_item"))) {
		...
		com.nightbeam.remnants.RemnantBossesMod.queueServerWork(80, () -> {
			...
			// spawn boss
			net.minecraft.world.entity.Entity boss = com.nightbeam.remnants.init.ModEntities.REMNANT_OSSUKAGE.get().spawn(_s,
					pos, net.minecraft.world.entity.MobSpawnType.MOB_SUMMONED);
			if (boss != null) {
				boss.setDeltaMovement(0, 0, 0);
				// Trigger initial spawn logic
				com.nightbeam.remnants.procedures.OssukageOnInitialEntitySpawnProcedure.execute(_s, boss);
			}
			// spawn minions
			for (int i = 0; i < (int) com.nightbeam.remnants.config.JaumlConfigLib.getNumberValue("remnant/bosses",
					"ossukage", "on_spawn_skeletons"); i++) {
				net.minecraft.world.entity.Entity min = com.nightbeam.remnants.init.ModEntities.SKELETON_MINION.get().spawn(
						_s,
