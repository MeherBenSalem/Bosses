...
public class RemnantOssukageEntity extends Monster {
	...
	public void tick() {
		...
		if (this.level().isClientSide()) {
			EntityTickEvents.updateOssukageAnimations(this);
		} else {
			EntityTickEvents.updateOssukageServerTick(this);
	...
	private void updateBossMusic() {
		...
		// Don't run music logic for dead/removed entities â€” prevents restart after die()
		if (!this.isAlive())
			return;

		// Hysteresis: Start at configured radius, stop at radius + 15
		int startRadius = (int) com.nightbeam.remnants.config.JaumlConfigLib.getNumberValue("remnant/bosses", "ossukage",
				"boss_music_radius");
